...
const STATUS = [
  "Resultado verificable por cualquiera",
  "Un boleto gratis en cada sorteo",
  "Envio a toda LATAM incluido",
];
...
export function HeroLine() {
  ...
  useEffect(() => {
    const hold = setTimeout(() => setOn(false), 3200);
    const next = setTimeout(() => {
      setI((n) => (n + 1) % STATUS.length);
      setOn(true);
    }, 3600);
    return () => {
      clearTimeout(hold);
